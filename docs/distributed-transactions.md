...
Старая архитектура lab1/lab2 (два datasource в одном Spring Boot процессе с Narayana/JTA-координатором) использовалась для академической демонстрации XA между двумя локальными БД. В lab3 этот подход не масштабируется на микросервисную модель и **полностью удалён** из кода. Он упоминается здесь только как контраст: то что работало в монолите с двумя БД, не работает между сервисами.
